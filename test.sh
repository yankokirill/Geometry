...
if g++ --version > /dev/null; then
    touch test.cpp
    TestStage "sh ./build.sh -DCMAKE_CXX_COMPILER=g++ -DCMAKE_C_COMPILER=gcc" "gcc build complete" "gcc build failed"
...
if clang++ --version > /dev/null; then
    touch test.cpp
    clang_cmd="sh ./build.sh -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_C_COMPILER=clang"
